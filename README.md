...
The file store remains independent: it performs no lifecycle registration and retains its existing byte format and migration rules. Consumers choose explicit configuration boundaries. Dawnwalker menu integrations prepare missing preferences at startup so their controls are available before the first save. Callbacks use committed values and perform no file I/O or periodic reconciliation.
...
Sessions may pass `{settings=adapter,loadSettings=read}` to `new`. A configuration boundary without a retained snapshot calls the optional reader, validates its result, and preserves the previous snapshot on failure. Entries receive the latest numeric snapshot in `SaveLoadContext.settings` and install `Session.onSettings(callback)` for live changes. Paused/closed sessions receive no updates; the adapter retains the newest values for the next session. `Session.restart()` schedules a guarded replacement for whole-mod enable/disable only and cancels net-zero enablement bursts. Ordinary changes should update cached values and wake the consumer's existing bounded worker. `Session.restore(key)` conditionally restores one owned field with readback verification; repeated toggles reuse its journal entry.

Subscription failures never trigger automatic retries, including invalid subscription return values. The snapshot adapter reports each distinct error once, retaining at most 64 error messages. An invalid callback payload is rejected as a whole; a listener failure retains the latest desired snapshot so a later lifecycle event can recover.
